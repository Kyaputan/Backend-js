...
const { addBookSchema , idBookSchema , updateBookSchema } = require('../validators/valbook');
const { requireLogin } = require('../middleware/auth');
const { checkRole } = require('../middleware/auth');

router.post('/addbook', requireLogin,validate(addBookSchema),checkRole("admin"), addBook );
router.delete('/deletebook/:id',requireLogin, validate(idBookSchema) ,checkRole("admin") , deleteBook );
